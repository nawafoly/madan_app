...
const reportDir = path.join(repoRoot, "reports", "hr-notifications-audit-migration");
const DEFAULT_BATCH_SIZE = 100;
const EXPECTED_WORKER_RELEASE = "phase7-notifications-audit-v1";
...
async function postBatch(apiUrl, syncSecret, body) {
  const response = await fetch(`${apiUrl.replace(/\/$/, "")}/internal/hr/notifications-audit/import`, {
    method: "POST",
    headers: {
  ...
  if (!response.ok || !payload?.ok) {
    throw new Error(`Cloudflare notifications/audit import failed (${response.status}): ${JSON.stringify(payload)}`);
  }
  return payload;
}
...
async function main() {
  ...
  const [notificationsSnapshot, auditSnapshot] = await Promise.all([
    db.collection("notifications").get(),
    db.collection("audit_logs").get(),
  ]);
  const notifications = notificationsSnapshot.docs.map(doc => ({ id: doc.id, ...serialize(doc.data() || {}) }));
  const auditLogs = auditSnapshot.docs.map(doc => ({ id: doc.id, ...serialize(doc.data() || {}) }));
  const runId = `hr-notifications-audit-${new Date().toISOString().replace(/[:.]/g, "-")}-${stableHash(`${notifications.length}:${auditLogs.length}`)}`;
  const report = { runId, projectId, generatedAt: new Date().toISOString(), dryRun, notifications: notifications.length, auditLogs: auditLogs.length };
  fs.mkdirSync(reportDir, { recursive: true });
  const reportPath = path.join(reportDir, `${runId}.json`);
  fs.writeFileSync(reportPath, JSON.stringify(report, null, 2), "utf8");
  console.log(JSON.stringify({ notifications: notifications.length, auditLogs: auditLogs.length }, null, 2));
  console.log(`[hr-notifications-audit] Report: ${reportPath}`);
  if (dryRun) {
    console.log("[hr-notifications-audit] Dry run completed. No Cloudflare data was changed.");
    return;
  }

  const health = await waitForWorkerRelease(apiUrl);
  console.log(`[hr-notifications-audit] Worker release active: ${health.release}`);

  const notificationChunks = chunks(notifications, batchSize);
  const auditChunks = chunks(auditLogs, batchSize);
  const max = Math.max(notificationChunks.length, auditChunks.length, 1);
  for (let index = 0; index < max; index += 1) {
    const notificationBatch = notificationChunks[index] || [];
    const auditBatch = auditChunks[index] || [];
    const payload = await postBatch(apiUrl, syncSecret, {
      runId,
      notifications: notificationBatch,
      auditLogs: auditBatch,
      complete: index === max - 1,
    });
    console.log(`[hr-notifications-audit] Batch ${index + 1}/${max}: notifications=${payload.notifications}, auditLogs=${payload.auditLogs}`);
  }
  console.log(`[hr-notifications-audit] Completed run ${runId}.`);
}

main().catch(error => {
  console.error("[hr-notifications-audit] Failed:", error);
  process.exitCode = 1;
});
