...
const reportDir = path.join(repoRoot, "reports", "hr-tasks-reports-migration");
const DEFAULT_BATCH_SIZE = 100;
const EXPECTED_WORKER_RELEASE = "phase8a-tasks-reports-v1";
...
async function postBatch(apiUrl, syncSecret, body) {
  const response = await fetch(`${apiUrl.replace(/\/$/, "")}/internal/hr/tasks-reports/import`, {
    method: "POST",
    headers: {
  ...
  if (!response.ok || !payload?.ok) {
    throw new Error(`Cloudflare tasks/reports import failed (${response.status}): ${JSON.stringify(payload)}`);
  }
  return payload;
}
...
async function main() {
  ...
  const [dailySnapshot, weeklySnapshot] = await Promise.all([
    db.collection("daily_tasks").get(),
    db.collection("weekly_reports").get(),
  ]);
  const dailyTasks = dailySnapshot.docs.map(doc => ({ id: doc.id, ...serialize(doc.data() || {}) }));
  const weeklyReports = weeklySnapshot.docs.map(doc => ({ id: doc.id, ...serialize(doc.data() || {}) }));
  const runId = `hr-tasks-reports-${new Date().toISOString().replace(/[:.]/g, "-")}-${stableHash(`${dailyTasks.length}:${weeklyReports.length}`)}`;
  const report = { runId, projectId, generatedAt: new Date().toISOString(), dryRun, dailyTasks: dailyTasks.length, weeklyReports: weeklyReports.length };
  fs.mkdirSync(reportDir, { recursive: true });
  const reportPath = path.join(reportDir, `${runId}.json`);
  fs.writeFileSync(reportPath, JSON.stringify(report, null, 2), "utf8");
  console.log(JSON.stringify({ dailyTasks: dailyTasks.length, weeklyReports: weeklyReports.length }, null, 2));
  console.log(`[hr-tasks-reports] Report: ${reportPath}`);
  if (dryRun) {
    console.log("[hr-tasks-reports] Dry run completed. No Cloudflare data was changed.");
    return;
  }

  const health = await waitForWorkerRelease(apiUrl);
  console.log(`[hr-tasks-reports] Worker release active: ${health.release}`);

  const dailyChunks = chunks(dailyTasks, batchSize);
  const weeklyChunks = chunks(weeklyReports, batchSize);
  const max = Math.max(dailyChunks.length, weeklyChunks.length, 1);
  for (let index = 0; index < max; index += 1) {
    const dailyBatch = dailyChunks[index] || [];
    const weeklyBatch = weeklyChunks[index] || [];
    const payload = await postBatch(apiUrl, syncSecret, {
      runId,
      dailyTasks: dailyBatch,
      weeklyReports: weeklyBatch,
      complete: index === max - 1,
    });
    console.log(`[hr-tasks-reports] Batch ${index + 1}/${max}: dailyTasks=${payload.dailyTasks}, weeklyReports=${payload.weeklyReports}`);
  }
  console.log(`[hr-tasks-reports] Completed run ${runId}.`);
}

main().catch(error => {
  console.error("[hr-tasks-reports] Failed:", error);
  process.exitCode = 1;
});
