...
const HR_WORKER_RELEASE = "phase9e-a1-permissions-attendance-v1";
...
function canViewPayroll(requester) {
  return (
    ...
    requester.permissions.includes("financial.view")
  );
}
...
function canManagePayroll(requester) {
  return (
    ...
    requester.permissions.includes("financial.edit")
  );
}

function canManageNotifications(requester) {
  return (
    requester.permissions.includes("settings.manage") ||
    requester.permissions.includes("employees.manage")
  );
}
...
function canViewAudit(requester) {
  return (
    requester.permissions.includes("settings.manage") ||
    requester.permissions.includes("reports.view")
  );
}
...
async function listEmployeeDirectory(db) {
  try {
    const rows = await db
      .prepare(
        `WITH employee_directory_source AS (
           SELECT
             e.*,
             COALESCE(
               NULLIF(TRIM(e.auth_uid), ''),
               (
                 SELECT a2.uid
                 FROM accounts a2
                 WHERE a2.linked_employee_id = e.id
                 ORDER BY a2.is_active DESC, a2.updated_at DESC
                 LIMIT 1
               )
             ) AS resolved_uid
           FROM employees e
         ...
         SELECT
           e.id,
           e.resolved_uid AS auth_uid,
           CASE
             WHEN LENGTH(TRIM(COALESCE(e.name, ''))) > 0
               AND LOWER(TRIM(e.name)) <> LOWER(TRIM(e.id))
               AND LOWER(TRIM(e.name)) <> LOWER(TRIM(COALESCE(e.resolved_uid, '')))
               THEN TRIM(e.name)
             ...
             ELSE 'موظف غير مرتبط'
           END AS name,
           COALESCE(NULLIF(TRIM(e.email), ''), a.email) AS email,
           e.avatar_url,
           COALESCE(NULLIF(TRIM(e.title), ''), a.title) AS title,
           e.department,
           e.employment_status,
           e.employee_code,
           e.allowed_zone_ids_json
         FROM employee_directory_source e
         LEFT JOIN accounts a ON a.uid = e.resolved_uid
         WHERE e.is_active = 1
           AND e.resolved_uid IS NOT NULL
           AND (a.is_active IS NULL OR a.is_active = 1)
         ORDER BY name COLLATE NOCASE ASC, e.id ASC
         LIMIT 500`
      )
      .all();
...
function canReviewDailyTasks(requester) {
  return requester.permissions.includes("daily_tasks.manager_notes");
}

function canReviewWeeklyReports(requester) {
  return requester.permissions.includes("weekly_reports.manager_notes");
}
