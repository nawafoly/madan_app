...
export function normalizeEmployeePayrollRecord(
  ...
  return {
    ...
    mudadDocument,
    status: String(raw.status || "finalized").trim() || "finalized",
    finalizedAt: raw.finalizedAt ?? null,
    finalizedByUid: String(raw.finalizedByUid || "").trim() || null,
    reopenedAt: raw.reopenedAt ?? null,
    reopenedByUid: String(raw.reopenedByUid || "").trim() || null,
    reopenReason: String(raw.reopenReason || "").trim() || null,
    revision: Math.max(1, toFiniteNumber(raw.revision) || 1),
    paidAt: raw.paidAt ?? null,
    paidByUid: String(raw.paidByUid || "").trim() || null,
